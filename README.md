...
```python forget_index_generate.py <dataset name> <train set size> <forget set size>```

For example for the problem of forgeting $10\%$ of the training set in cifar10, we use the following command:

```python forget_index_generate.py cifar 50000 5000```
...
2. Then we collect the results from all the unlearning indices for each retrained model. As an example, for retrain model ```ResNet18_orig__10```, we use the following command (it automatically replace ```1,10,100``` for the unlearning indices and computes the average and std):

```python summarize_mia_retrain.py logs/scratch/cifar_ResNet18/unlearn/retrain/5000/unl_idx_seed_1/ResNet18_orig__10/200_mia_SVC_results.csv```
